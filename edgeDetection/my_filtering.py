...
def my_filtering(img, kernel, boundary = 0):
    ...
    kernelu = np.flipud(np.fliplr(kernel))
    ...
    for i in range(img.shape[1]):
        for j in range(img.shape[0]):
            ...
            filtered_img[j,i] = (kernelu*pad_image[j:j+ksizeY,i:i+ksizeX]).sum()

    return filtered_img

src = cv2.imread('Lena.png', 0) #TA 사용 이미지 ( 4000 x 3000 )
cv2.imshow('fff', src)

gaus2D = my_getGKernel((5,5), 5)
gaus1D = my_getGKernel((1,5), 5)
