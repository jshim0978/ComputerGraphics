...
def my_DoG(img, ksize, sigma = 1, gx = 0, boundary = 0): #default (3,3) sigma = 1, y축 편미분
    ...
    if gx == 0:
        DoG = (-y / (sigma ** 2)) * np.exp(-(y * y) / (2 * sigma * sigma))
    elif gx == 1:
        DoG = (-x / (sigma ** 2)) * np.exp(-(x * x) / (2 * sigma * sigma))
...
LoG = my_LoG(gray, 15)
DoGX = my_DoG(gray, 15, sigma = 3, gx = 1, boundary = 2)
DoGY = my_DoG(gray, 15, sigma = 3, gx = 0, boundary = 2)
